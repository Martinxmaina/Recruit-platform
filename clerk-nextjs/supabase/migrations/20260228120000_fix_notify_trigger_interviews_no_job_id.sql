-- Fix: interviews table has no job_id column; only applications has it on the row.
-- Stop reading NEW.job_id for interviews; job_id for interviews is set via SELECT from applications.
...
begin
  ...
  -- Only read job_id from NEW/OLD for tables that have this column (applications only; interviews has application_id)
