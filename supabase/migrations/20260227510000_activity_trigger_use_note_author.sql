-- Use note author_id/author_name for activity log when table is notes, so "added notes" shows the user.
...
BEGIN
  ...
  -- Prefer note author from row; else app context; else system/tracked_candidates fallback
  IF TG_TABLE_NAME = 'notes' AND NEW.author_id IS NOT NULL THEN
    user_id_val := NEW.author_id;
    user_name_val := COALESCE(NEW.author_name, 'Unknown');
  ELSE
...
COMMENT ON FUNCTION public.create_activity_log() IS 'Automatically creates activity log entries for changes to applications, interviews, notes, candidates, and tracked_candidates. Uses note author for notes; otherwise app user context.';
